...
export function MobileInsightSummary({ state, game }: { state: PreActionInsightState; game: GameState }) {
  ...
  const baselineSummary = state.analysis?.sections.find((section) => section.kind === "baseline")?.text;
  const coachSummary = state.liveCoach
    ? `当前${state.liveCoach.hero.currentHand}${state.liveCoach.hero.upgrades[0]
      ? ` · ${state.liveCoach.hero.upgrades[0].name}${percent(state.liveCoach.hero.upgrades[0].byRiver)}`
      : ""}`
    : undefined;
