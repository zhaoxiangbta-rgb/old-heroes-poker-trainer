...
import { buildCoachFacts, reviewInsightInput } from "./coachFacts";
...
function reviewAction(action: StrategyAction, decision: DeepDecisionInput): DeepDecisionReviewV3["candidates"][number] {
  return {
    action: action.action === "fold" || action.action === "check" || action.action === "call"
      ? { type: action.action }
      : { type: "raise", to: action.toAmount ?? decision.legal.minRaiseTo },
    ev: action.ev,
...
export async function calculateDeepHandReview(
  ...
  const decisions: DeepDecisionReviewV3[] = [];
  ...
  for (const decision of [...input.decisions].sort((a, b) => a.logIndex - b.logIndex)) {
    ...
    const ranges: DeepDecisionReviewV3["ranges"] = {};
    ...
    progress("equity-ev");
    const insightInput = reviewInsightInput(decision);
    const publicRanges = inferOpponentRanges(insightInput);
    const strategy = createLocalStrategyEngine().decide({
      state: publicDecisionStateFromInsight(insightInput),
      ranges: {
        version: 1,
        lastActionIndex: insightInput.logIndex,
        bySeat: Object.fromEntries(publicRanges.map((range) => [range.seat, range.ranges.map((combo) => ({
    ...
    const strategyCandidates = strategy.actions.map((action) => reviewAction(action, decision));
    const alignedCalculation = {
      ...calculation,
      candidates: strategyCandidates,
      confidence: Math.min(calculation.confidence, strategy.confidence),
    };
    const actual = actualOrFallback(decision);
    const recommended = [...strategyCandidates].sort((a, b) => b.ev - a.ev)[0];
    const actualCandidate = [...strategyCandidates].sort(
    ...
    const tags = [] as DeepDecisionReviewV3["tags"];
    ...
    const corrections = normalizedEvLoss > 0.03
      ...
      : [];
    const baseCoach = buildCoachFacts({
      decision,
      calculation: alignedCalculation,
      rangesBySeat,
    ...
    const narrative = buildCoachNarrative({
      facts: {
        madeHandLabel: baseCoach.madeHandLabel,
        heroRangePercentile: baseCoach.heroRangePercentile,
        equityVsFullRange: baseCoach.equityVsFullRange,
        equityVsContinueRange: baseCoach.equityVsContinueRange,
        opponentBuckets: baseCoach.opponentBuckets,
        opponentResponses: baseCoach.opponentResponses,
        atLeastOnePlayerBehindContinues: baseCoach.atLeastOnePlayerBehindContinues,
        runoutSummary: baseCoach.runoutSummary,
        confidence: baseCoach.confidence,
      },
    ...
    const analysis = buildPlainLanguageAnalysis({
      input: insightInput,
      exact: decision.preActionInsight?.exact,
      ranges: publicRanges,
      responses: decision.preActionInsight?.responses ?? [],
      strategy,
      sampleBudget: callbacks.config.sampleBudget,
    });
    decisions.push({
      ...
      confidence: alignedCalculation.confidence,
      tags: [...new Set(tags)],
      correctThinking,
      corrections,
      ...
      coach: { ...baseCoach, ...narrative },
      analysis,
      opponentRanges: publicRanges.map((range) => ({
        playerId: range.playerId,
        comboCount: range.comboCount,
        buckets: { ...range.buckets },
        latestAction: range.changes.at(-1) ?? "尚无足够行动信息",
  ...
  progress("saving");
  const wholeHand = buildWholeHandNarrative(decisions, input.decisions);
  return {
    version: 3,
    status: "completed",
    handNo: input.handNo,
