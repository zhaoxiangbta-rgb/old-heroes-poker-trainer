...
function rankScore(hole: readonly Card[], board: readonly Card[]): number {
  ...
  let rankMask = 0;
  for (let index = 0; index < hole.length + board.length; index += 1) {
    const card = index < hole.length ? hole[index] : board[index - hole.length];
...
function universalNutsStatus(heroHole: readonly [Card, Card], board: readonly Card[]): "absolute" | "tied" | "not" {
  const hero = rankScore(heroHole, board);
  const deck = availableDeck([...heroHole, ...board]);
  let tied = false;
  for (let first = 0; first < deck.length - 1; first += 1) {
    for (let second = first + 1; second < deck.length; second += 1) {
      const opponent = rankScore([deck[first], deck[second]], board);
      const comparison = opponent - hero;
...
export function calculateExactProjection(
  ...
  for (let index = 0; index < finals.length; index += 1) {
    ...
    const status = universalNutsStatus(input.heroHole, board);
